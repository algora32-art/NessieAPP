...
type WorkOrder = {
  id: string;
  status: string; // status_key (ej: "Nuevo")
  client_id: string;
  client_name: string;
  phone: string;
  address: string;
...
export default function Kanban() {
  ...
  async function loadMeAndTechs() {
    ...
    const { data: prof } = await supabase
      ...
      .eq("id", user.id)
      .maybeSingle();
    setMeRole((prof?.role ?? "technician") as "admin" | "technician");
  ...
  async function createOS() {
    ...
    const scheduled_start = scheduledLocal
  ...
  const safeStatuses: StatusRow[] = statuses.length
    ? statuses
    : [
        {
          key: "Nuevo",
  ...
  const byStatus = (key: string) => items.filter((i) => i.status === key);
      ...
      <Card className="p-4">
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-4">
          ...
          {meRole === "admin" ? (
            ...
            <div className="md:col-span-2">
              <div className="text-xs text-[rgb(var(--muted))] mt-6">
                * La asignación la realiza un administrador.
              </div>
            </div>
          )}
        </div>
        ...
        <div className="mt-4">
          <Button onClick={createOS} disabled={!clientName || !phone || !address}>
            Crear OS
          </Button>
        </div>
      </Card>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            <div className="text-sm text-[rgb(var(--muted))]">
              Arrastra (próximo) / cambia el estado desde el selector.
            </div>
            ...
            {safeStatuses.map((st) => (
              <div key={st.key} className="min-w-0">
                ...
                <div className="space-y-3">
                  {byStatus(st.key).map((wo) => (
                    <Card key={wo.id} className="p-3">
                        ...
                        >
                          ...
                          {!compact ? (
                            <>
                              ...
                              {wo.assigned_to_name ? (
                                <div className="mt-2 text-[11px] rounded-full border px-2 py-0.5 inline-block">
                                  Téc: {wo.assigned_to_name}
                                </div>
                              ) : null}
                      ...
                      <div className="mt-3">
                        ...
                        <Select
                          value={wo.status}
                          onChange={(e) => move(wo.id, e.target.value)}
                        >
                          {safeStatuses.map((s2) => (
                  ...
                  {!byStatus(st.key).length && (
                    <div className="text-xs text-[rgb(var(--muted))]">Sin OS</div>
                  )}
                </div>
              </div>
            ))}
